...
export async function processRecurringBilling() {
  ...
  for (const invoice of due) {
    try {
      ...
      await q(
        `INSERT INTO payment_transactions (id, tenant_id, amount, description, provider, type, status, payment_url, provider_transaction_id, created_at)
         VALUES ($1, $2, $3, $4, $5, 'subscription', 'pending', $6, $7, NOW())`,
        [txId, invoice.tenant_id, invoice.amount,
         `Subscription: ${invoice.plan_name} (${invoice.billing_cycle})`,
         payment.provider, payment.paymentUrl || '', payment.transactionId || '']
      );

      if (payment.success && !payment.offline) {
        await q(
          `UPDATE subscriptions SET current_period_start = current_period_end,
            current_period_end = current_period_end + INTERVAL '1 ${invoice.billing_cycle === 'annual' ? 'year' : 'month'}'
           WHERE id = $1`,
          [invoice.sub_id]
        );
        await q(
      ...
      results.push({ tenant_id: invoice.tenant_id, plan: invoice.plan_code, amount: invoice.amount, status: payment.success ? 'processed' : 'pending', payment_url: payment.paymentUrl });
    } catch (e) {
