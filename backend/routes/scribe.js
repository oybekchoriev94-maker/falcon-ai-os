...
// XAVFSIZLIK: `transcribe()` (STT) va `llm()` (Ollama) ikkalasi ham
// so'rov ICHIDA `fetch()` orqali tashqi xizmatga chiqadi (STT klinika
// GPU'siga, LLM Ollama'ga — ikkalasi ham SSH tunnel orqali). Node'ning
// ichki fetch (undici) katta/uzoq javoblarda AsyncLocalStorage tenant
// kontekstini (authMiddleware o'rnatgan) UZIB QO'YISHI mumkin ekan —
// natijada fetch'dan KEYINGI DB yozuvlari tenant kontekstisiz ishlab,
// RLS "new row violates row-level security policy" bilan yiqilardi
// (patient_consultations'ga yozishda aniqlandi). SELECT'lar esa jim
// bo'sh natija qaytargani uchun bu ilgari sezilmagan.
//
// Yechim: har bir tashqi fetch()'dan KEYIN, DB yozuvlariga o'tishdan
// oldin kontekstni ANIQ qayta tiklaymiz — kontekst haqiqatan uzilgan
// yoki uzilmaganidan qat'i nazar xavfsiz (uzilmagan bo'lsa mos tenant
// bilan mos kelib hech narsa o'zgarmaydi).
...
export default function scribeRoutes(pool, authMiddleware, checkRole, upload, serverError, logger) {
  ...
  router.post('/transcribe', authMiddleware, checkRole('doctor', 'admin'), upload.single('audio'), async (req, res) => {
    try {
      ...
      if (req.file) {
        ...
        if (stt.error) {
          // fetch() dan keyin — kontekstni qayta tiklab, keyin yozamiz
      ...
      // STT fetch()'dan keyin kontekstni qayta tiklaymiz — bu blok ichida
      // llm() ham fetch() qiladi, shu sababli DB yozuvidan oldin YANA
      // qayta tiklaymiz (ichki runWithTenantDbContext).
      return runWithTenantDbContext(tenantId, async () => {
        ...
        // llm() fetch()'idan keyin kontekstni yana tiklaymiz — DB yozuvlari shu ichda.
        return runWithTenantDbContext(tenantId, async () => {
  ...
  router.post('/upload', authMiddleware, upload.single('audio'), async (req, res) => {
    try {
      ...
      // STT fetch()'dan keyin kontekstni qayta tiklaymiz.
      return runWithTenantDbContext(tenantId, async () => {
        await markTranscribed(pool, rec?.id, text);
        ...
        return runWithTenantDbContext(tenantId, async () => {
          ...
          // Telegram xabarnomasi ham fetch() — yakuniy yozuvdan oldin kontekstni yana tiklaymiz.
          return runWithTenantDbContext(tenantId, async () => {
            await q(`INSERT INTO usage_metering (tenant_id, metric, count, date) VALUES ($1, 'ai_requests', 1, CURRENT_DATE) ON CONFLICT (tenant_id, metric, date) DO UPDATE SET count = usage_metering.count + 1`, [tenantId]);
            trackAiRequest('scribe', tenantId);
